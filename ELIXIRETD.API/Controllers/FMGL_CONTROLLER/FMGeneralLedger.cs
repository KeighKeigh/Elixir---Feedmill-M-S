...
{
    [Route("api/fm-gl"), ApiController]
    [AllowAnonymous]
    public class FMGL : ControllerBase
    {
        private readonly IMediator _mediator;
        public FMGL(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        [HttpGet]
        [ApiKeyAuth]
        public async Task<IActionResult> Get([FromQuery] FMGLQuery query)
        {
        ...
        public class FMGLQuery : IRequest<Result<List<FMGLResult>>>
        {
            public string adjustment_month { get; set; }

        }

        public class FMGLResult
        {
        ...
        public class Handler : IRequestHandler<FMGLQuery, Result<List<FMGLResult>>>
        {
            ...
            public async Task<Result<List<FMGLResult>>> Handle(FMGLQuery request, CancellationToken cancellationToken)
            {
                if (string.IsNullOrWhiteSpace(request.adjustment_month))
                {
                    return Result.Success(new List<FMGLResult>());
                }
                ...
                var result = consolidateList.SelectMany(x => new List<FMGLResult>
                {
                    
                    //debit
                    new FMGLResult
                    {
                        SyncId = "FM&S-" + (x.SyncId.ToString() ?? string.Empty) + "-D",
                        Mark1 = string.Empty,
                        Mark2 = string.Empty,
                        ...
                        BOA2 = "Inventoriables",
                        System = "Elixir - Feedmill M&E",
                        Books = "Journal Book",
                    },
                    //credit
                    new FMGLResult
                    {
                        SyncId = "FM&S-" + (x.SyncId.ToString() ?? string.Empty) + "-C",
                        Mark1 = string.Empty,
                        Mark2 = string.Empty,
                        AssetCIP = x.AssetCIP ?? string.Empty,
                        AccountingTag = string.Empty,
            ...
            private async Task<List<FMGLResult>> MoveOrderTransactions(DateTime startDate, DateTime endDate)
            {
            ...
            private async Task<List<FMGLResult>> ReceiptTransactions(DateTime startDate, DateTime endDate)
            {
            ...
            private async Task<List<FMGLResult>> IssueTransactions(DateTime startDate, DateTime endDate)
            {
            ...
            private async Task<List<FMGLResult>> BorrowedTransactions(DateTime startDate, DateTime endDate)
            {
            ...
            private async Task<List<FMGLResult>> ReturnedTransactions(DateTime startDate, DateTime endDate)
            {
            ...
            private async Task<List<FMGLResult>> FuelTransactions(DateTime startDate, DateTime endDate)
            {
